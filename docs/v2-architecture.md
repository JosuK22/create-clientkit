...
### Stage 58 — the resolved stack is recorded (landed)

The change Stage 57 sanctioned, and nothing else.

#### What changed

`.client-site.json` now records the stack the project resolved to:
...
Three production edits, each small:

- `generate/provenance.ts` gains `ResolvedStack` - a `Pick` of `ProjectManifest`,
  not a restatement of it - and `buildProvenance` takes it as an argument.
- `generate/plan.ts` gains `PlanOptions.stack`, **required**. The planner sees a
  `ProjectContext`, which describes the site and the template but not the stack;
  deriving one there would hand back V1's Astro constants for every project.
  Required rather than optional, because a provenance file that sometimes
  records the stack and sometimes does not is two formats wearing one name.
- `adapters/bridge.ts` passes it from the manifest the pipeline already agreed
  on.

The starter and the feature list are deliberately not in `stack`. Both are
already recorded, as `mode` and `config.features`, and a document holding each
of them twice would eventually hold two different answers.

#### The golden diff, reviewed

**30 snapshots changed. 270 insertions. Zero deletions.** Every added line is
one of three things: `"stack": {`, one of the seven dimensions, or the closing
`},` - 30 × 9, audited mechanically rather than by eye.

| Snapshot                                                       | Change                                                                   |
| -------------------------------------------------------------- | ------------------------------------------------------------------------ |
| `coming-soon-url.txt`                                          | +9 / -0                                                                  |
| `full-url.txt`                                                 | +9 / -0                                                                  |
| `url-less.txt`                                                 | +9 / -0                                                                  |
| `cli-resolution.txt`                                           | **unchanged** - it renders a `ProjectContext`, which holds no provenance |
| 27 V2 snapshots (React, Next, Bootstrap, MUI, router, feature) | +9 / -0 each                                                             |

The 27 were expected from the same cause: those snapshots also capture the
generated `.client-site.json`. No template output moved, no ordering changed,
nothing was reformatted.

#### The checksum moved, deliberately

```text
was:  812c438185ecb2317cc5d741e7f83f1a06750f2a83e9b22551205eb60d4dbc0d
now:  f73e1f8ab5092e3fd522742198e51574d64588109d54151a6cf1d905097dec20
```

This is the first time the V1 golden checksum has changed since it was
recorded, and it changed because Stage 57 decided it could: `.client-site.json`
is ClientKit-owned metadata, not immutable template output, and
`docs/golden-snapshots.md` requires exactly this - an agreed product change,
reviewed through the snapshot diff. The earlier stage entries in this document
quote the old value; they are records of what was true when they were written
and are left alone.

Worth noting for whoever inherits this: no test, script or CI job asserts the
checksum. It has been a discipline rather than a gate, which is why changing it
had to be a decision someone took rather than a red test someone fixed.

#### A boundary that had to be named

`test/domain.test.ts` asserts the V1 generation path does not import the domain
layer, and `generate/provenance.ts` now imports `ProjectManifest` from it. The
file joins the allow-list that `ui/plan.ts` and `context/explain.ts` already
sit on, for the same reason they do: it imports the type and nothing else. The
exemption is narrower than theirs - a new test asserts every domain import in
that file is `import type`, so the entry cannot widen into a runtime dependency
without failing. An erased type import changes neither the bundle nor a byte of
generated output, which is what the guard exists to protect.

The alternative was to restate the seven dimensions as local string unions,
which would have been a second description of a model that already exists and
free to drift from it. A `Pick` cannot drift.

#### What this does not do

No upgrade command, no provenance reader, no migration, no file deletion, no
ownership record, no argument-parser change. Projects generated by 1.0.2 - every
project that exists today - still carry no stack, and a future upgrade must
refuse them by name rather than guess.

### Stage 59 — reading provenance back (landed)

A reader, and nothing that acts on what it reads. `src/domain/provenance-reader.ts`
answers one question - _what does this document prove about this project?_ - and
deliberately not the next one, which belongs with the command that would do
something about it.

#### Readable is not sufficient

The distinction the module exists for. Every project generated up to and
including 1.0.2 has a provenance file that is well-formed, understandable, and
silent about the stack. It is **readable** and **not sufficient**, and those are
different answers:

| Document                            | Status                              |
| ----------------------------------- | ----------------------------------- |
| Current provenance, stack recorded  | `usable`                            |
| 1.0.2 provenance, no stack          | `insufficient` (`missing: 'stack'`) |
| No `.client-site.json`              | `missing`                           |
| Not JSON                            | `malformed`                         |
| JSON, but not a provenance document | `invalid`                           |
| Written by a newer ClientKit        | `unsupported`                       |

Collapsing the first two into one boolean is how a tool starts guessing, and
Stage 56 measured the cost: a missing stack rebuilt from resolver defaults turns
a project that chose MUI into one that never did, and a developer's own module
into a file the tool believes it owns. So there is no default, no inference and
no repair anywhere in the reader. A document that does not say something is
reported as not saying it - and a test asserts the `insufficient` result
mentions no framework, styling system or architecture the document did not.

#### Shape

A discriminated union rather than a thrown error, following `validateTargetDir`,
whose own comment makes the same point: several of these are ordinary answers
rather than failures. A project with no provenance is not a malfunction.

Validation order matters and is deliberate. The version gate runs immediately
after `cliVersion` is confirmed to be a string, _before_ any structural check,
because a document from a newer ClientKit may legitimately have a shape this
build would otherwise call invalid - blaming the document for this CLI's age
would be the wrong answer. Stage 57 settled the direction: a newer CLI must
understand an older document, and the reverse is explicitly unsupported.

#### Where it lives, and why

In `src/domain/`. It validates a document against the domain vocabulary - the
seven dimension id lists, reused rather than restated - and answers a domain
question. It imports no filesystem: the file reader is injected, as `--from`
already does it, and a read that throws is taken as "no document", the same
reading `validateTargetDir` makes of a failed `statSync`.

It keeps its own copy of the file name rather than importing the writer's,
because a domain module reaching into `generate/` would invert the dependency
direction the project has held since Stage 2. A test asserts the two constants
are identical, so the duplication cannot become a disagreement.

#### What it will not do

No writing, no generating, no repair, and no looking at any file other than the
one it was asked for - asserted by counting the paths a fake reader is asked
for, so a future fallback to `package.json` would fail rather than pass quietly.

**Generated output is untouched.** No golden moved; the checksum is still
`f73e1f8ab5092e3fd522742198e51574d64588109d54151a6cf1d905097dec20`.

#### Known limitations

1. Nothing calls the reader. It has no consumer until an upgrade planner exists,
   which is the next stage's problem - the reader was written first on purpose,
   because deciding what a document proves is a smaller question than deciding
   what to do about it, and mixing them is how the second one gets decided by
   accident.
2. Projects generated by 1.0.2 remain `insufficient` permanently. No later
...
### Stage 64 — the upgrade command (landed)

Nine stages of contract, and the command itself is mostly an order of
operations. Stage 59 reads the document, Stage 62 validates its template
identity, Stage 63 computes the two path sets, Stage 55's merge writes planned
files and touches nothing else. This stage decides what order those run in,
what sentence the developer reads, and when they are asked.

```bash
npm create clientkit@latest upgrade ./my-site
npm create clientkit@latest upgrade ./my-site --styling bootstrap
npm create clientkit@latest upgrade ./my-site --dry-run
```

#### The promise, unchanged from Stage 61

> ClientKit re-generates the files it would generate today for this project's
> recorded stack, using the configuration you supply now. It lists every file
> it would replace and asks before replacing any of them. It never deletes a
> file, and it never touches a file it did not plan.
>
> When your stack changes, it also reports which previously generated files the
> new stack no longer produces - and leaves them exactly where they are.
>
> It does not know what a previous version of ClientKit generated, and it does
> not claim to.

#### `upgrade` is a command, not a directory

Stage 56 found that `create-clientkit upgrade ./site` created a project called
`upgrade`. The parser now takes a command word off the front before the
directory rule applies. The cost is that a directory literally named `upgrade`
can no longer be created by bare name; `create-clientkit ./upgrade` still
works, and that is the trade every subcommand makes.

#### Where the configuration comes from

The recorded document is layered _underneath_ the developer's input, as a
config-file layer, so the existing precedence does the work and no second rule
was invented:
...
recorded, but Bootstrap". The site name and URL are not restated, because the
project already recorded them.

**Configuration never prompts.** An upgrade is not a fresh creation flow, so
the resolver runs against a non-interactive prompter: the document answers what
the project already decided, a flag answers what the developer wants changed,
and anything genuinely missing fails with that sentence rather than reopening
the whole interview. The only question the command asks is the one that
matters.

**A framework change restarts the stack's derived half.** A recorded Astro
project carries `buildTool: astro`, `router: file-based` and
`architecture: astro-standard`; carrying those into `--framework react` would
hand the compatibility engine a stack no framework offers, and the developer
would be told their own reasonable request was incompatible. What survives is
what does not belong to a framework - styling, component library, language,
features - and the rest is re-derived exactly as a new project would derive it.

#### Consent is separate from configuration

`--yes` is refused outright, rather than honoured. Stage 55 measured the
existing contract: `--yes` selects a prompter that declines to answer a
write-into-a-non-empty-directory question at all, so a second `--yes` run
already refuses today. An upgrade always targets a directory full of a
developer's work, and treating `--yes` as consent would widen that boundary in
the one command where it matters most. A non-TTY run refuses for the same
reason: the point of the confirmation is that a person saw the list.

`--dry-run` needs neither, because it writes nothing - the consent prompter is
constructed only at the point a write is about to happen.
...
This upgrade would:
  ...
  Replace 21 generated file(s):
    ...

  Add 1 file(s):
    src/components/ui/AppProviders.tsx

  Leave 2 file(s) no longer generated by this stack:
    src/components/ui/AppProviders.tsx
    src/routes/AppRouter.tsx

  These are not deleted. Remove them yourself once you have checked them.

Files ClientKit did not plan are never touched.
```

Orphan candidates are listed under what they are. They are never called files
to delete, because ClientKit does not delete them and saying so would describe
a different tool. A test asserts the output matches neither `files to delete`
nor `will be deleted`.

#### Two defects this found

**The merge path was not atomic.** `apply()`'s fast path - a fresh directory -
publishes with a single rename and always was. The merge path could not, since
the target keeps files the plan does not name, so it moved planned files in one
at a time, deleting each original first. A failure on the twentieth file left
nineteen replaced and their originals gone. Stage 64 found it by failing an
upgrade on purpose and watching `.client-site.json` - which sorts early - come
back describing a stack the project no longer had.

Each replaced file is now moved aside into the staging tree and restored if any
later move fails, so the target ends up either fully updated or exactly as it
was. That closes hard stop 10: a failed application cannot leave provenance
claiming success.

**A directory at a planned path was silently destroyed.** Once originals were
moved aside rather than deleted, a directory sitting where a planned file goes
would have been moved aside too - and discarded with the staging tree, taking
whatever it held. It now refuses before touching anything:

```text
Cannot write "src/App.tsx": a directory exists at that path.
```

Both fixes are in `apply()` and so apply to `create`'s confirmed merge as well,
which had the same exposure.

#### Provenance afterwards

`.client-site.json` is part of the current plan, so a confirmed upgrade rewrites
it with the stack that now exists - by construction rather than by a special
mutation path. A declined or failed upgrade leaves it byte-identical. Tests
cover all three, plus upgrading twice in a row, where the second run's baseline
...
| Outcome                                                    | Code |
| ---------------------------------------------------------- | ---- |
| Upgraded, or declined, or dry run                          | `0`  |
| Missing, malformed, unsupported or insufficient provenance | `2`  |
| Template identity incompatible with the recorded stack     | `2`  |
| `--yes`, or no terminal, when consent is required          | `2`  |
| Application failure                                        | `1`  |

Declining is not an error: the developer was asked a question and answered it.

#### What it does not do

No deletion - the command's source contains no `rmSync`, `unlinkSync`,
`rmdirSync` or `renameSync`, asserted by test. No ownership inference: it reads
exactly one file itself, the provenance document, through an injected reader,
and never inspects generated output to decide anything. No historical
migration. No path arithmetic of its own - a test asserts it calls the Stage 63
planner and computes no difference itself, so a second copy cannot disagree
with the one the tests cover.

#### Known limitations

1. **The site description is not recorded** (Stage 61, Decision B), so an
   upgrade regenerates it from the template default unless `--from` supplies
   one. The file list will not show this, because content changes move no path.
2. **No post-steps.** A stack change rewrites `package.json`, but the upgrade
   does not install. Run the install yourself afterwards.
3. **`--yes` cannot upgrade**, by design, which means the command has no
   unattended mode. That is a deliberate consequence of consent being a
   person's job, and it makes upgrade unusable in CI.
4. **Template-version drift stays invisible.** Both plans render today's
   templates, so a file whose content ClientKit changed between releases is
