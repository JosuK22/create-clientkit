...
### Upgrading a project ClientKit generated
...
configuration is the baseline, so `--styling bootstrap` means "everything as
before, but Bootstrap" — you do not restate the site name, the URL or the rest
of the stack.

It lists every file it would replace, every file it would add, and every file
your new stack no longer generates, and then asks. What it promises:

- **Nothing is ever deleted.** Files the new stack no longer generates are
  reported and left exactly where they are, for you to remove once you have
  looked at them.
...
- **Confirmation is required.** `--yes` is refused rather than treated as
  consent, and a run without a terminal refuses too. `--dry-run` needs neither,
  because it writes nothing.
- **A failure changes nothing.** Every replaced file is restored if any later
  write fails, so the project ends up either fully updated or exactly as it was.

It refuses, without guessing, when the project has no `.client-site.json`, when
that file is malformed, when it came from a newer ClientKit, when it records no
stack (every project generated by 1.0.2), or when its template identity
contradicts its stack.

**One thing to know before you confirm.** Your site description is not recorded
in `.client-site.json`, so an upgrade writes ClientKit's default in its place.
The file list will not show this, because the description changes content
rather than which files exist — so the command warns you before asking, and
names the wording it would write. To keep your own, put it in a config file and
pass `--from`:

```json
{ "site": { "description": "Hand-built orreries for discerning collectors." } }
```

`upgrade` reconciles your recorded configuration against the **current**
templates. It does not know what an earlier ClientKit generated and does not
claim to, so a file whose content changed between releases is reported as
unchanged. It also runs no install: after a stack change, `package.json` is
rewritten and you install yourself.

### Re-running ClientKit on a project it already generated

Running `create` again over an existing project is a merge, not an upgrade, and
it has not changed:
...
Re-running with a different stack adds and overwrites but leaves the previous
stack's files where they are: drop `--ui-library mui` and
`src/components/ui/AppProviders.tsx` stays, still importing a package the new
`package.json` no longer lists. The build tree-shakes it away and succeeds;
`npm run typecheck` does not and fails. `upgrade` is the command that tells you
