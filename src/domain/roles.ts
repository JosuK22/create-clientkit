...
export const FILE_ROLES = [
  ...
  'app.layout',
  /**
   * The composed part of the document head.
   *
   * Distinct from `app.layout`, which is the shell the framework's template
   * owns and which Stage 39 proved cannot be edited without changing V1's
   * generated bytes. This is the surface a future realization renders *into* -
   * a file that exists only when something contributes to it, imported and
   * rendered by the shell at a place the architecture declares.
   *
   * Mapping it is not a claim that anything fills it. An architecture that maps
   * it says "there is a place in my document where composed head entries
   * belong, and it is here"; with no contributions nothing is generated and
