...
describe('which metadata features compose into a file', () => {
  it('SEO composes exactly its two document components', () => {
    /*
     * The decision the old version of this test asked for. It measured that no
     * metadata feature changed a single generated byte, and said that if a
     * later stage built a real composition mechanism this was the test that
     * should fail first - and that its failure would be good news, provided it
     * was a decision somebody made rather than drift nobody noticed.
     *
     * Stage 44 is that decision. SEO now states a document, so it generates the
     * components that render it. The measurement did not go away; it narrowed
     * to exactly what composing is allowed to add.
     */
    const without = filesOf(base());
    const withSeo = filesOf(base({ features: ['seo'] as FeatureId[] }));

    const added = [...withSeo.keys()].filter((file) => !without.has(file)).sort();
    expect(added).toEqual([
      'src/components/DocumentHead.astro',
      'src/components/DocumentHeadPageNotFound.astro',
    ]);
    expect([...without.keys()].filter((file) => !withSeo.has(file))).toEqual([]);
  });

  it('changes only the files the composition renders through', () => {
    // The shell gains a head slot, the not-found page gains its fragment, and
    // the template component gives up the field that moved. Nothing else.
    const without = filesOf(base());
    const withSeo = filesOf(base({ features: ['seo'] as FeatureId[] }));
    const changed = [...withSeo.entries()]
      .filter(([file, content]) => without.has(file) && without.get(file) !== content)
      .map(([file]) => file)
      .sort();
  ...
  it('accessibility and structured data still compose nothing', () => {
    // Neither states a document, so neither changes a byte. The original
    // measurement survives intact for the features it still holds for.
    const without = filesOf(base());
    for (const feature of ['accessibility', 'structured-data'] as FeatureId[]) {
      const withOne = filesOf(base({ features: [feature] }));
